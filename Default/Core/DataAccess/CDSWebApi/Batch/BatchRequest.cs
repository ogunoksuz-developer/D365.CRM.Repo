...
namespace PowerApps.Samples.Batch
{
    public class BatchRequest : HttpRequestMessage
    {
        ...
        /// </summary>
        public List<ChangeSet> SetChangeSets
        {
        ...
        /// </summary>
        // NOSONAR
        public List<HttpRequestMessage> SetRequests
        {
            get => throw new NotImplementedException();
            set
            {
        ...
        private HttpMessageContent ToMessageContent(HttpRequestMessage request)
        {
            ...
            {
                if (request.Content.Headers.Contains(ContentTypeHeader))
                {
                    request.Content.Headers.Remove(ContentTypeHeader);
                }
                request.Content.Headers.Add(ContentTypeHeader, "application/json;type=entry");
            }

            HttpMessageContent messageContent = new(request);

            if (messageContent.Headers.Contains(ContentTypeHeader))
            {
                messageContent.Headers.Remove(ContentTypeHeader);
            }
            messageContent.Headers.Add(ContentTypeHeader, "application/http");
